$(document).ready(function () {
	...
	$(function () {
	    ...
	    var advertise = $('#advertise-js'),
	        advertiseWrap = advertise.find('.advertise__block-img'),
	        advertiseWrapWidth = advertiseWrap.width(),
	        advertiseWrapHeight = advertiseWrap.height(),
	        advertiseImage = advertiseWrap.find('img');

	        // цикл для создание эффекта background-size: cover для images
	        for (var i = 0; i < advertiseImage.length; i++) {// проходим по всем полностью img во всех блоках
	            if (advertiseWrapWidth / advertiseWrapHeight >= 1 && advertiseImage.eq(i).width() / advertiseImage.eq(i).height() > 1) {// если ширина блока деленая на высоту блока равна или больше 1 и ширина img деленная на высоту img больше 1 тогда:
	                advertiseImage.eq(i).css({// задаем параметры
	                    height: '100%',
	                    width: 'auto'
	                });
	            } else{
	                advertiseImage.eq(i).css({
	                    height: 'auto',
	                    width: '100%'
	                });
	            }
	        ...
	        var addImages = function (element, number) {
	            ...
	            element.children('.append-line_' + number + '').eq(0).addClass('advertise__img-active');// добавляем активный клас для отображения первой линии слайдера

	            // функция изменяющая активные классы для линии и images
	            var toggleClassActive = function (data) {
	                element.children('.append-line_' + number + '').removeClass('advertise__img-active');// удаляем у всех дивов с классом append-line(2 или 3) активный класс
	                element.children('.append-line_' + number + '').eq(data).addClass('advertise__img-active');// добавляем определенному классу append-line(2 или 3) активный класс
	                element.children('img').removeClass('advertise__img-active');// удаляем у всех img активный класс
	                element.children('img').eq(data).addClass('advertise__img-active');// добавляем определенному img активный класс
	            };
	            ...
	            element.mousemove(function (argument) {
	                ...
	                if(x < advertiseWrapWidth / number) {// когда курсор находиться в первой части блока по оси Х
	                ...
	                if (x < (advertiseWrapWidth / number) * 2 && x > advertiseWrapWidth / number) {// когда курсор находиться во второй части блока по оси Х
	                ...
	                if (x > (advertiseWrapWidth / number) * 2 && boolian) {// когда курсор находиться в третей части блока по оси Х
	    ...
	    // цикл проверяющий наличие и количество img в блоках (.advertise__img)
	    for (var i = 0; i < advertiseWrap.length; i++) {
	        advertiseWrap.eq(i).children(':first').addClass('advertise__img-active');// во всех блоках даем img активный класс
	        if (advertiseWrap.eq(i).find('img').length >= 3) {// если количество img в блоке равно или больше 3
	            addImages(advertiseWrap.eq(i), 3);// тогда запускаем функцию addImages и передаем первым параметром обьект, в котором найдено количество img равное или больше 3 и вторым параметром длину масива максимальную 3 
	        } else if (advertiseWrap.eq(i).find('img').length == 2) {// если количество img в блоке равно 2
	            addImages(advertiseWrap.eq(i), 2);// тогда запускаем функцию addImages и передаем первым параметром обьект, в котором найдено количество img равное 2 и вторым параметром длину масива 2 
	        } else if(advertiseWrap.eq(i).find('img').length == 0){// если в блоке нет img
	            advertiseWrap.eq(i).css('background', '#000');// тогда даем ему фон черный
	        }
	    }
	...
	// end mini slide
	    ...
	    $(function () {
				...
				var	paginationContent = $('#pagination__item-js'),
	    			...
	    			listAdvertiseSum = 20,// показывающее количество элементов с рекламой
	    			remainder = 1;

	    	// общая функция определяющая показ количества рекламных блоков и навигации
	    	function paginations() {
	    		
	    		var listAdvertise = $('#advertise-js').children('.advertise__block'),// все блоки с рекламой
	    			listAdvertiseLength = listAdvertise.length;// общая длина блоков с рекламой

	    		if (listAdvertiseLength % listAdvertiseSum == 0) {
	    			remainder = 0;
	    		}

	    		var listAdvertiseFloor = Math.floor(listAdvertiseLength / listAdvertiseSum) + remainder;// округление в меньшую сторону показа общего количества навиганионных блоков 

	    		// проверка если общее количество рекламных блоков больше заданного количества показываемых блоков
	    		if (listAdvertiseLength > listAdvertiseSum) {
	    			for (var i = 0; i < listAdvertiseFloor; i++) {// запускаем цикл до округленного числа в меньшую сторону общего количества навигационных блоков
				...
				function paginationActives(boolian) {
					...
					listAdvertise.show();
					listAdvertise.eq(index * listAdvertiseSum).prevAll().hide();
					listAdvertise.eq(index * listAdvertiseSum).nextAll().slice(listAdvertiseSum - 1).hide();
				};
